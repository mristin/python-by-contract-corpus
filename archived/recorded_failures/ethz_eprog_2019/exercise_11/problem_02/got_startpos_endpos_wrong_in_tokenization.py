...
class Token(DBC):
    ...
    # TOKENIZATION_MAP was {<TokenKind.NUM: 1>: <correct_programs.ethz_eprog_2019.exercise_11.problem_02.TokenizationRule object at 0x000001EA37449F40>, <TokenKind.VAR: 2>: <correct_programs.ethz_eprog_2019.exercise_11.problem_02.TokenizationRule object at 0x000001EA37310700>, <TokenKind.OP: 4>: <correct_programs.ethz_eprog_2019.exercise_11.problem_02.TokenizationRule object at 0x000001EA37474700>, <TokenKind.OPEN: 5>: <correct_programs.ethz_eprog_2019.exercise_11.problem_02.TokenizationRule object at 0x000001EA37474910>, <TokenKind.CLOSE: 6>: <correct_programs.ethz_eprog_2019.exercise_11.problem_02.TokenizationRule object at 0x000001EA37474820>, <TokenKind.WHITESPACE: 7>: <correct_programs.ethz_eprog_2019.exercise_11.problem_02.TokenizationRule object at 0x000001EA37474970>}
    # TOKENIZATION_MAP[kind].pattern was re.compile('[a-zA-Z_][a-zA-Z_0-9]+')
    # TOKENIZATION_MAP[kind].pattern.fullmatch(text[startpos:endpos]) was None
    # endpos was 23
    # kind was <TokenKind.VAR: 2>
    # startpos was 0
...
def _parse_atom(tokens: TokensWoWhitespace, cursor: int) -> Tuple[Expr, int]:
    ...
    if remaining >= 2 and tokens[cursor].value == "-":
        ...
        raise SyntaxError(
            ...
            f"at column {tokens[cursor].startpos + 1}; expected an atom"
        )
...
def parse_tokens(tokens: Sequence[Token]) -> Expr:
    ...
    if endpos != len(tokens_wo_ws):
        raise SyntaxError(
            f"Expected end of source, "
            f"but got token {tokens[endpos].value!r} "
            f"of kind {tokens[endpos].kind!r} "
            f"at column {tokens[endpos].startpos + 1}"
        )
...
@ensure(lambda expr, result: parse_tokens(tokens_to_text(result)) == expr)
