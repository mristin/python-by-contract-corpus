...
class Token(DBC):
    ...
    def __repr__(self) -> str:
        ...
        return (
            ...
            f"{self.text!r}, {self.start}, {self.end}, {self.kind!r})"
        )
...
# parse_program(tokenize(result)) was Program([Assign('A', BinaryOperation(Constant(1.0), '+', Variable('inf')))])
# program was Program([Assign('A', BinaryOperation(Constant(1.0), '+', Constant(inf)))])
# result was 'A = (1.0)+(inf);\n'
# tokenize(result) was [Token('A = (1.0)+(inf);\n', 0, 1, <TokenKind.VAR: 2>), Token('A = (1.0)+(inf);\n', 1, 2, <TokenKind.WHITESPACE: 7>), Token('A = (1.0)+(inf);\n', 2, 3, <TokenKind.ASSIGN: 8>), Token('A = (1.0)+(inf);\n', 3, 4, <TokenKind.WHITESPACE: 7>), Token('A = (1.0)+(inf);\n', 4, 5, <TokenKind.OPEN: 5>), Token('A = (1.0)+(inf);\n', 5, 8, <TokenKind.NUM: 1>), Token('A = (1.0)+(inf);\n', 8, 9, <TokenKind.CLOSE: 6>), Token('A = (1.0)+(inf);\n', 9, 10, <TokenKind.OP: 4>), Token('A = (1.0)+(inf);\n', 10, 11, <TokenKind.OPEN: 5>), Token('A = (1.0)+(inf);\n', 11, 14, <TokenKind.VAR: 2>), Token('A = (1.0)+(inf);\n', 14, 15, <TokenKind.CLOSE: 6>), Token('A = (1.0)+(inf);\n', 15, 16, <TokenKind.SEMICOLON: 9>), Token('A = (1.0)+(inf);\n', 16, 17, <TokenKind.WHITESPACE: 7>)]
#
# Falsifying example: execute(
#     kwargs={'program': Program([Assign('A', BinaryOperation(Constant(1.0), '+', Constant(inf)))])},
# )
